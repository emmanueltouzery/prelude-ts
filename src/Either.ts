...
import {
  WithEquality, areEqual,
  hasTrueEquality, getHashCode
} from "./Comparison";
...
export class EitherStatic {
   ...
   * call with `lift<L,R>(...)`.
   *
   *     const add = Either.lift((x:number,y:number) => x+y);
   *     add(1,2);
   *     => Either.right(3)
   *
   ...
   *     => Either.left("x")
   */
  lift<T extends any[], U, L = unknown>(fn: (...args: T) => U): (...args: T) => Either<L, U> {
    return (...args: T) => {
      try {
        const r = fn(...args);
        if (r !== undefined) {
...
export class Left<L, R> implements Value {
  ...
  mapT<U>(fn: (x: R) => Task<U>): Task<Either<L, U>> {
    return Task.of(() => Promise.resolve(this as any))
  }
  /**
  ...
  mapLeftT<U>(fn: (x: L) => Task<U>): Task<Either<U, R>> {
    return fn(this.value).map(r => new Left<U, R>(r))
  }
  /**
...
export class Right<L, R> implements Value {
  ...
  mapLeftT<U>(fn: (x: L) => Task<U>): Task<Either<U, R>> {
    return Task.of(() => Promise.resolve(<any>this))
  }
  /**
   * Map the either: you give a function to apply to the value,
   * a function in case it's a left, a function in case it's a right.
