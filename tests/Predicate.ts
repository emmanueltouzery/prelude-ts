...
describe("predicate composition", () => {
    it("predicate and composition", () =>
       assert.ok(Predicate.lift(x=>x>2).and(x=>x<10)(5)));
    it("predicate and composition 2", () =>
       assert.ok(!Predicate.lift(x=>x>2).and(x=>x<10)(11)));
    it("predicate or composition", () =>
       assert.ok(!Predicate.lift(x=>x<2).or(x=>x>10)(5)));
    it("predicate or composition 2", () =>
       assert.ok(Predicate.lift(x=>x<2).or(x=>x>10)(11)));
    it("predicate negation", () =>
       assert.ok(Predicate.lift(x=>x<10).negate()(20)));
    it("predicate allOf", () =>
